#!/usr/bin/env python3
"""Generate 50 articles - English with full content, translations with translated metadata + English body"""
import os, json, re
...
RELS = [
    ...
    ['cannabis-terpenes-explained','Cannabis Terpenes','Understanding terpenes and effects.'],
...
if os.path.exists(DATA_FILE):
    with open(DATA_FILE) as f: all_arts = json.load(f)
    for art in all_arts:
        langs = {}
        ...
        write_art(art['slug'], langs)
    print(f"\nGenerated {len(all_arts)} articles x 5 languages = {len(all_arts)*5} files!")
else:
    print("No data file. Run data generation first.")
